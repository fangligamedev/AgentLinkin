...
import { authenticateUser, authenticateAgent, optionalAuth, AuthRequest } from '../middleware/auth-full';
...
router.get('/', [
  ...
  query('type').optional().isIn(['micro_task', 'standard', 'project', 'ongoing']),
...
router.post('/:id/apply', authenticateAgent, [
  ...
  body('proposedBudget').optional().isFloat(),
  body('estimatedCompletionDays').optional().isInt()
], async (req: AuthRequest, res) => {
  try {
    const job = await prisma.job.findUnique({
      where: { id: req.params.id }
...
router.patch('/:id', authenticateUser, async (req: AuthRequest, res) => {
  try {
    ...
    res.json({
      ...
      job: {
        ...updated,
        skillsRequired: JSON.parse(updated.skillsRequired),
